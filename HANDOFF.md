...
`BKTC_Ledger`（pywebview 桌面应用，**v1.1.12**）是**当前唯一的编辑入口**；`BKTC上海POU营业管理表.records.json` 是**权威数据源**；`build_ledger_main.py` 是**生成引擎**（已并入工具,自包含）；`BKTC上海POU营业管理表.xlsx` 是**生成产物**（42 sheet）。**打包走 GitHub Actions**(win+mac CI);飞书/钉钉已同步(见 §6)。

> v1.1.5–v1.1.12 新增：任意六表**筛选+排序**、**单表导出 Excel**(📥)、**跨表查询导出**(🔍 report builder，按客户等任意关联字段筛+选字段+导出)、发货批次**拆分**(⇲)、**3 处派生字段自动同步**(付款条件文本/应收回款日/验收状态，见 §8)、**JOB No 列筛选跨表同步**(v1.1.10)、**新建订单弹窗**(v1.1.11–v1.1.12：四区表单一次填→自动落六表；付款条件行编辑器款类+比例；建单后六表占位；合同页级联删除)。
...
### 2026-08-07 v1.1.10（JOB 跨表筛选同步）
- **JOB No 列筛选跨表同步**:六表唯一共有列 `JOB No` 的列筛选从「按表存储」(`colFilters[table]["JOB No"]`)提升为**全局共享**(`state.jobFilter`)。任一表(如合同订单)筛某 JOB→切到发货批次/开票/回款等页自动跟随,免重复筛选;列筛选弹窗标题标注「此列跨表共享」,表头 tabHint 显示当前跨表 JOB;「✕ 清除筛选/排序」一并清全局 JOB;导出 Excel 随 `visibleRows` 自动跟随。关键字搜索本就全局、未改;客户/款类等列各表不同,无法跨表。

### 2026-08-07 v1.1.11–v1.1.12（新建订单弹窗 + 六表占位 + 级联删除）
- **新建订单弹窗**(顶栏「＋新建订单」):四区表单(订单头/设备明细/发货/付款条件),一次填→push 进 `state.data` 对应表(纯前端,无后端 api)。JOB 号 `suggestJobNo` 自动建议当年 BS 最大+1(可切 DS);客户/担当者/型号/地点/PO/款类用 `<datalist>` 可选现有或新建;设备按型号×台数展开 N 行占位;提交后 `recomputeDerived`+`state.jobFilter=新JOB`+`switchTab("合同订单")` 自动跳转筛选。
- **付款条件行编辑器**:每行=款类(`datalist` 现有5种 预付/发货/到货/验收/质保款 +可新建)+比例%+账期天数;`＋添加条款`/`✕`增删;「填A/B/C模板」快捷填充;`nocTermSummary` 实时合计(=100% 绿✓);全留空则建一条空占位。
- **六表占位**:建单后 付款条件/开票记录/回款记录 若该 JOB 无记录则各建 1 条空占位(`{记录ID,JOB No}`),保证切表都有该 JOB 行、免手动添加;`validateAll` 付款比例合计跳过空款类行(占位不触发 ≠100% error);空製造番号校验 error→warning(对齐 core.py)。
- **合同级联删除**:合同订单页删行→`tryCascadeDelete` 两次 `confirm`→`Object.keys(state.schema)` 过滤删该 JOB 在六表全部数据;其他表删除不变。
- **本地记录ID**:`local-<ts>-<计数器>`,计数器防同批多行撞键(`feishu_upsert` 回写按 `(表,记录ID)` 匹配);`core.normalize/derive/generate_xlsx` 对非空 local- ID 安全。⚠️空占位行在 `feishu_upsert` 同步时也会推送,建议填好或删空行再同步。
