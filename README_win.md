...
设置页的“检查更新”检查的是 GitHub Release，不是普通的 `main` 分支 push。发布新版本时需要先提交代码，再创建版本标签（本次为 `v1.7.0`），GitHub Actions 才会生成 Windows zip。Windows 端点击“下载并打开”会下载并解压到 `下载\BKTC_Ledger_update`，然后请退出正在运行的旧程序，用新目录替换旧程序目录；当前不会在运行中的 exe 上自动覆盖更新。
