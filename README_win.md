...
设置页的“检查更新”检查的是 GitHub Release，不是普通的 `main` 分支 push。发布新版本时需要先提交代码，再创建版本标签（本次为 `v1.6.0`），GitHub Actions 才会生成 Windows zip。Windows 端点击“下载并打开”会下载并解压到 `下载\BKTC_Ledger_update`，然后请退出正在运行的旧程序，用新目录替换旧程序目录；当前不会在运行中的 exe 上自动覆盖更新。
...
报价单在应用内保存到同一个 SQLite 数据库。选择来源 JOB 可带入客户、型号、数量、单价和商务条款；“历史报价”只查询同客户同型号的已保存记录。导出文件位于当前用户的 `下载\报价单\`，不需要也不能把报价 XLSX 关联为数据源。
